...
void setup() {
  ...
  if (EEPROM.read(0)!=99 || EEPROM.read(1)==0 || EEPROM.read(2)==0) {EEPROM.write(0,99);EEPROM.write(1,25);EEPROM.write(2,1);}  //first time run setup
                                                                                                                                //when address 0 in EEPROM is not 99, then write 99.
                                                                                                                                //25 on address 1 for the voice volume and 1 on address 2 for the Folder.
  VolumeSet=EEPROM.read(1); //Read adress 1 and set Volume variable
  FolderSet=EEPROM.read(2); //Read adress 2 and set Folder variable    
  VolumeVoice(VolumeSet); //Set Volume
  digitalWrite(Led,HIGH); //Power LED on when init is ready
}
...
if(ResultMatrix > 0 && ResultMatrix < 5 && ((unsigned long)(millis() - time_NotInUse)) > 2000){SetVolume();}  //when button pushed longer then 2 sec, jump to the set voice volume
if(ResultMatrix == 16 && ((unsigned long)(millis() - time_NotInUse)) > 5000){SetFolder();}  //when button pushed longer then 2 sec, jump to the set voice volume
if(ResultMatrix == 0 && Block == true) {Block=false;PlayVoice(ButtonMemory);} //on key release deblock new key press and play song selected
